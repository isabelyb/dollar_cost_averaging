...
df = pd.read_csv('https://raw.githubusercontent.com/alnunez/crypto_coins_historical_data/main/Binance_BTCUSDT_d.csv', skiprows=[0])

'''LIMPIAR DATOS'''


# Eliminar columnas irrelevantes
df.drop(['unix',	'symbol',	'high',	'low',	'Volume USDT',	'Volume BTC',	'tradecount'], axis=1, inplace=True)
...
plt.title('Number of time with negative price variation per week for BTC\n24/06/2020 to 25/06/2021')
plt.ylabel('Times')
plt.show()
...
plt.title('% Price Variation in BTC\nfrom 24/06/2020 to 25/06/2021\n')
plt.show()

# Variación en el tiempo (negativa)
df_neg.plot.line('date','% var', color='r', legend=False).set_title('% Negative Price Variation in BTC\nfrom 24/06/2020 to 25/06/2021\n')
plt.show()
